...
public class ViewDragHelper {
    ...
    private void clearMotionHistory(int pointerId) {
        ...
        if (pointerId >= 0 && pointerId < mInitialMotionX.length){
            mInitialMotionX[pointerId] = 0;
        }
        if (pointerId >= 0 && pointerId < mInitialMotionY.length){
        ...
        if (pointerId >= 0 && pointerId < mInitialEdgeTouched.length){
            mInitialEdgeTouched[pointerId] = 0;
        }
        if (pointerId >= 0 && pointerId < mEdgeDragsInProgress.length){
            mEdgeDragsInProgress[pointerId] = 0;
        }
        if (pointerId >= 0 && pointerId < mEdgeDragsLocked.length){
    ...
    private void saveLastMotion(MotionEvent ev) {
        ...
        for (int i = 0; i < pointerCount; i++) {
            ...
            if(pointerId<mLastMotionY.length) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                for (int i = 0; i < pointerCount; i++) {
                    ...
                    float tempdy = 0;
                    if (pointerId >=0 && pointerId < mInitialMotionX.length){
                        tempdx = mInitialMotionX[pointerId];
                    }
                    if (pointerId >=0 && pointerId < mInitialMotionY.length){
                        tempdy = mInitialMotionY[pointerId];
                    }
                    final float dx = x - tempdx;
                    final float dy = y - tempdy;
                ...
                if (mDragState == STATE_DRAGGING) {
                    ...
                    try {
                        ...
                        if(mActivePointerId >=0 && mActivePointerId < mLastMotionX.length){
                            idx = (int) (x - mLastMotionX[mActivePointerId]);
                        }
                        if(mActivePointerId >=0 && mActivePointerId < mLastMotionX.length){
                            idy = (int) (y - mLastMotionX[mActivePointerId]);
                        }
    ...
    private boolean checkNewEdgeDrag(float delta, float odelta, int pointerId, int edge) {
        ...
        int initialEdge = 0;
        if (pointerId >= 0 && pointerId < mInitialEdgeTouched.length){
            initialEdge = mInitialEdgeTouched[pointerId];
        }
        int edLocked = 0;
        if (pointerId >= 0 && pointerId < mEdgeDragsLocked.length){
            edLocked = mEdgeDragsLocked[pointerId];
        }
        int edProgress = 0;
        if (pointerId >= 0 && pointerId < mEdgeDragsInProgress.length){
            edProgress = mEdgeDragsInProgress[pointerId];
        }
        if ((initialEdge & edge) != edge || (mTrackingEdges & edge) == 0
                || (edLocked & edge) == edge
                || (edProgress & edge) == edge
        ...
        return (edProgress & edge) == 0 && absDelta > mTouchSlop;
    }
    ...
    public boolean checkTouchSlop(int directions, int pointerId) {
        ...
        float initialy = 0;
        if (pointerId >=0 && pointerId < mLastMotionX.length){
            lastdx = mLastMotionX[pointerId];
        }
        if (pointerId >=0 && pointerId < mLastMotionY.length){
            lastdy = mLastMotionY[pointerId];
        }
        if (pointerId >=0 && pointerId < mInitialMotionX.length){
            initialx = mInitialMotionX[pointerId];
        }
        if (pointerId >=0 && pointerId < mInitialMotionY.length){
            initialy = mInitialMotionY[pointerId];
        }
        final float dx = lastdx - initialx;
        final float dy = lastdy - initialy;
    ...
    public boolean isEdgeTouched(int edges, int pointerId) {
        ...
        return isPointerDown(pointerId) && (initialedge & edges) != 0;
    }
